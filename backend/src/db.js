...
export function ensureSchema(handle = db) {
  ...
  if (hasUsersTable) {
    ...
    try {
      ...
      const defaultPwd = process.env.DEFAULT_ADMIN_PASSWORD || '@dmin5066';
      const defaultHash = bcrypt.hashSync(defaultPwd, 12);
      if (!existingSuperAdmin) {
        handle.prepare(`
          ...
          VALUES ('Admin', 'dipu@populardiagnostic.com', ?, 'super_admin', ?, 'Administrator', 'EMP001', 1, 'active', 0)
        `).run(defaultHash, superGroup?.id || null);
        console.log('[SEED] Created default super admin. Email: dipu@populardiagnostic.com | Password: ' + defaultPwd);
      } else {
        ...
        handle.prepare("UPDATE users SET password_hash = ?, password_must_change = 0, is_active = 1, live_status = 'active', token_version = 0 WHERE id = ?").run(defaultHash, existingSuperAdmin.id);
        console.log('[SEED] Ensured default super admin password is set for dipu@populardiagnostic.com');
      }
    } catch (err) {
