...
function loadEnv() {
  ...
  if (!process.env.DEFAULT_ADMIN_PASSWORD) {
    process.env.DEFAULT_ADMIN_PASSWORD = '@dmin5066';
    const alreadySet = lines.some((l) => l.trim().startsWith('DEFAULT_ADMIN_PASSWORD='));
    if (!alreadySet) {
      lines.push(`DEFAULT_ADMIN_PASSWORD=@dmin5066`);
      try { fs.writeFileSync(ENV_PATH, lines.join('\n') + '\n'); } catch { /* best effort */ }
    }
  }
}
