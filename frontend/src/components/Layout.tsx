...
import {
  ...
  Radio,        FolderKanban, ChevronDown, FileText,
} from 'lucide-react';
import { useAuth } from '../lib/auth';
import { useTheme } from '../lib/theme';
import { api, getToken } from '../lib/api';
import { playNotificationSound } from '../lib/notificationSound';
import type { Notification, LiveStatusType, User } from '../lib/types';
import { timeAgo, cx, statusById } from '../lib/utils';
import { useSettings } from '../lib/settings';
import { trackNavigation } from '../lib/activityTracker';
...
export function Layout() {
  const { user, isAdmin, hasPermission, logout, setUser } = useAuth();
  ...
  useEffect(() => {
    ...
    if (user) {
      try {
        ...
        const sseUrl = token ? `/api/notifications/stream?token=${encodeURIComponent(token)}` : `/api/notifications/stream`;
        ...
        es.addEventListener('live-status', (e) => {
          try {
            const data = JSON.parse(e.data);
            setUser((prev: User | null) => prev ? { ...prev, live_status: data.live_status, last_active_at: data.last_active_at ?? prev.last_active_at } : prev);
          } catch {}
        });
      } catch {}
    }
  ...
  // Track navigation as activity for Live Status
  const location = useLocation();
  useEffect(() => {
    if (user) {
      trackNavigation(location.pathname, location.pathname);
    }
  ...
  const navGroups = useMemo(() => {
    const main = [
      ...
      { to: '/documents', label: 'DMS', icon: FileText, visible: true },
